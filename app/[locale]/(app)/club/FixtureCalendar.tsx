...
export default function FixtureCalendar({
  ...
  // Same crest-color extractor used across the club pages — the next-fixture
  // marker uses it instead of the generic accent, kept subtle (a dot + a
  // hairline) rather than a filled block so it doesn't compete with the row
  // content.
  ...
  // The list is one chronological timeline (oldest visible past -> nearest
  // future), so "next fixture" is simply the first future row — mark that
  // boundary instead of a separate "upcoming" table.
          ...
          <tbody>
            {rows.map((row, i) => (
              <Fragment key={row.id}>
                {i === nextFixtureIndex && (
                  <tr>
                    <td colSpan={5} className="border-t border-border/60 px-3 pb-1 pt-3">
                      <span
                ...
                <tr
                  className={`border-b border-border transition-colors last:border-b-0 ${
                    i === nextFixtureIndex
                      ? ""
                      : `${i % 2 === 1 ? "bg-foreground/[0.03]" : ""} ${row.finished ? "hover:bg-accent/5" : ""}`
                  }`}
                  style={
                ...
                <td className="whitespace-nowrap px-3 py-2 text-muted">
                  {row.finished ? (
                    <Link href={`/club/fixture/${row.id}`} className="hover:text-accent">
